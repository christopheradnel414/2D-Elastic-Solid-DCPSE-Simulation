...
for i in range(int(Nx/6)):
    xuppercircle.append(radius*np.cos((i)*np.pi/(Nx/6-1)) + xmid)
    yuppercircle.append(radius*np.sin((i)*np.pi/(Nx/6-1)) + ymid)
...
for i in range(1,int(Nx/6)-1):
    xlowercircle.append(radius*np.cos((i)*np.pi/(Nx/6-1)) + xmid)
    ylowercircle.append(-radius*np.sin((i)*np.pi/(Nx/6-1)) + ymid)
...
for i in range(1,Nx-1):
    for j in range(1,Ny-1):
        xtemp = i*h
        ytemp = j*h
        if (xtemp - xmid)**(2) + (ytemp - ymid)**(2) > (radius+htol*h)**(2) and (xtemp - xmid2)**(2) + (ytemp - ymid2)**(2) > (radius2+htol*h)**(2) and (xtemp - xmid3)**(2) + (ytemp - ymid3)**(2) > (radius3+htol*h)**(2):
            xinner.append(xtemp)
            yinner.append(ytemp)
            nxinner.append('nan')
            nyinner.append('nan')
